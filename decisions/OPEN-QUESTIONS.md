...
| Q77 | **Where should `EvidencePack` live?** It is defined in `app.services` and annotated in `app.prompts`, which sits one layer below — the only recorded exception in the layering contract. The import is already `TYPE_CHECKING`-guarded and `app.prompts` never constructs a pack, only renders one it is handed, but grimp resolves type-only imports so the guard alone does not satisfy the contract. Moving the pack to a layer both may see is a wide change touching every producer and consumer of it. Listed as `ignore_imports` rather than left broken, because a contract nobody can satisfy is one that gets switched off — which this file's own preamble warns about, and which is exactly what happened | `D-012`, the spine contract | Harshan | Open |

| Q78 | **How many acceptance cases have ever been judged?** A recorded transcript is keyed on a hash of its prompt and nothing on disk records which golden case produced it, so the eval gate cannot say how much of the acceptance set it actually covered. It reports coverage as *unknown* rather than as a count — a score over an unknown denominator is the `R10` failure in numeric form | `EV1`, the eval scorecard | Harshan | Open |
| Q79 | **Do we add DeepEval with a local judge, and accept a gate that needs the box?** DeepEval with a local Ollama judge is the recorded choice (`01-stack.md` §3), but a gate that needs a GPU runs once a burst — and the failures it exists to catch ship in between. The judge-shaped dimensions are therefore **declared and reported unmeasured on every run** rather than quietly left off the list. That is honest, and it is not a substitute | `EV2`, `EV4` | Harshan | Open |
| Q80 | **Should `audit_numbers` adopt the scorecard's wider character set?** The scorecard folds more dash, space and digit variants than the audit does, deliberately, so that where the two disagree a person looks rather than the gate silently deciding a correct answer was dishonest. Whether the audit should simply take the wider table is unanswered — widening it makes the gate more forgiving of the model, and that is a product decision | `EV2`, `audit_numbers` | Harshan | Open |
| Q81 | **Should a fit caveat the model writes itself count as disclosure?** `audit_fit_disclosed` checks that a poor fit is disclosed; when the answer offers its own caveat in different words, the dimension does not audit it. Accepting the model's own phrasing would let it satisfy the check by sounding careful | `EV3`, `audit_fit_disclosed` | Vishnu + Harshan | Open |
| Q82 | **How many steps may a multi-step task hold?** `C10` fixes no depth. Set to **8** in `app/agents/context.py`, matching `max_react_steps`, and flagged because a task step is **not** a ReAct step — one is a unit of a person's work across turns, the other a tool call inside one turn. Whether they should share a ceiling at all is the real question | `C10` | Harshan | Open |
| Q83 | **Should a conversation age faster than the case it concerns?** The task-memory staleness default borrows `RC9`'s seven days, which is a case-ageing figure rather than a conversation one. A conversation abandoned mid-task and a case nobody has touched are different kinds of neglect and may not deserve the same interval | `C10`, `RC9` | Harshan | Open |
| Q84 | **When the honesty payload alone exceeds the context ceiling, which gives way?** The gate outcome, the signal notes, the data window and the fault label are the four things that may never be dropped — so when they alone exceed `max_context_chars` there is nothing left to trim. The assembler returns them whole and unsent, and asks whether the right answer is a narrower question or a deliberately raised ceiling. Trimming the payload is not on the list | context assembly, `Q48` ceilings | Harshan | Open |
| Q85 | **Where does an approval request live?** `C9` builds one per response, so it exists only as long as the response does. Constraint 22 says a case must be able to go stale, and an approval nobody can find cannot age at all; an unassigned request that vanishes on refresh is indistinguishable from one never raised. The likely answer is `RC7`'s **authorisation work order** — the artefact that route already produces | `C9`, `RC7`, constraint 22 | Harshan | Open |
| Q86 | **Can the platform tell two people apart, or only two personas?** `C9` refuses a self-approval, but with `is_production_identity` hard-wired `False` (`Q41`) two holders of one persona are indistinguishable — so one signing the other's request reads as self-approval and is refused. Over-refusing is the cheap error, the same asymmetry as constraint 24, and the cost is a second person being asked. The real fix needs identity | `C9`, `G3`, `Q41` | Harshan | Open |
| Q87 | **What feeds the reliability workspace its detection list?** Constraint 21 is *detection is not seeding*, and `_seeding_gap` exists to prove it — but the `/workspace` handler derived the detector's list from the open cases themselves, so it compared the queue against itself and reported *"All N detected episode(s) have a case, checked against the detector"* having checked nothing. With zero cases open against the episodes the detector found, that false zero is the twenty-two-episodes failure the constraint was written for. The handler now supplies nothing and the surface says the check was not run, which is true but is not the check. Wiring the real detector output must also settle **which encoding of the constraint-35 triple is the identity** — `CaseRow.make_seed_key` joins equipment, fault and day with a pipe, `list_episodes` joins the same triple with a colon — or every episode compares as missing | `RC8`, `U6`, constraint 21, constraint 35 | Harshan | Open |
