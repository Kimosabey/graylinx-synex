...
|---|---|---|
| Repository access — `graylinx-synex` | Harshan | `git clone` and `git log` shows today's commits |
| Repository access — `thermynx` | Harshan | needed for the reference docs, which Synex's own docs cite |
| SSH key on the Jarvis box | Harshan | `ssh -o BatchMode=yes root@<box> "ollama --version"` answers without a password |
| Plant MySQL credentials | Harshan | `/api/v1/health` reports `plant_database.connected: true` |
| `backend/.env` | **not in git** — Harshan sends it | `SYNEX_MODEL_MODE=live` and the MySQL block are present |

**`.env` is gitignored deliberately.** It carries the plant credentials. Do not commit it, and do
