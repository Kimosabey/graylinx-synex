...
| 2026-08-11 | Two ID collisions found in the freshly-converted chapters and normalised on the way out of the .docx: design levels `L0`–`L6` (ch. 26) and architecture layers `L0`–`L9` (ch. 28) both collided with Alerts `L1`–`L6`, and release gates `G0`–`G5` (ch. 37) with Control Plane `G1`–`G6`. Now "Level n", "Layer n" and "Gate n". They had survived because the ID scanner treated `**` and `|` as non-boundaries, so a cell reading `| **G0 Design** |` was invisible — widened, and self-tested | Claude |
